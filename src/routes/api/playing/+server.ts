...
export const GET: RequestHandler = async () => {
	return await fetch(`${urls.api.lastfm}&user=${credentials.lastfm.username}&api_key=${credentials.lastfm.api_key}&format=json`).then(res => res.json()).then(async res => {
		const lastfm = res.recenttracks.track[0];
		const spotify = await api.search.get(`artist:${lastfm.artist["#text"]} ${lastfm.name}`, {
			include: {
				track: true
			},
			limit: 1,
